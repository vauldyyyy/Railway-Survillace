# youtube_mass_download.ps1
# RailGuard — Tier 3 Mass YouTube Data Harvester (PS version)
...
$OutputBase = "datasets/youtube_raw"
...
foreach ($Key in $SortedKeys) {
    ...
    $KeyDir = Join-Path $OutputBase $Key
    New-Item -ItemType Directory -Path $KeyDir -Force | Out-Null
    
    Write-Host "`n[!] Downloading: $QueryString" -ForegroundColor Cyan
    
    & yt-dlp "ytsearch5:$QueryString" `
        -f "best[height<=480][ext=mp4]" `
        -o "$KeyDir\%(title)s.%(ext)s" `
        --no-playlist `
        --ignore-errors `
        --sleep-interval 3 `
...
Write-Host "`n[DONE] Mass download sequence complete." -ForegroundColor Green
Write-Host "[LOG] Next step: python backend/scripts/extract_all_frames.py" -ForegroundColor Yellow
